...
contract WithdrawWithTypeReentrancyTest is MethodReentrancyTest {
    function callMethod() external {
        ...
        vm.prank(depositor);
        silo.withdraw(amount, depositor, depositor, ISilo.CollateralType.Protected);
    }
    ...
    function verifyReentrancy() external {
        ...
        vm.expectRevert(ICrossReentrancyGuard.CrossReentrantCall.selector);
        silo1.withdraw(1000, address(0), address(0), ISilo.CollateralType.Protected);
    }

    function methodDescription() external pure returns (string memory description) {
        description = "withdraw(uint256,address,address,uint8)";
    }
}
